...
fn convert_message(msg: anthropic::Message) -> Result<Vec<openai::Message>> {
            ...
            for block in blocks {
                match block {
                    ...
                    anthropic::ContentBlock::Image { source } => {
                        let data_url = format!("data:{};base,{}", source.media_type, source.data);
...
pub fn openai_to_anthropic(resp: openai::OpenAIResponse) -> Result<anthropic::AnthropicResponse> {
    let choice = resp
        .choices
        .first()
...
mod tests {
    ...
    #[test]
    fn test_clean_schema_removes_nested_uri_format() {
        let schema = json!({
            "type": "object",
            "properties": {
                "url": {
